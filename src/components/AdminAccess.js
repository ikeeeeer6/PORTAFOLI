...
export const AdminAccess = ({ cars, onAddCar, onDeleteCar, onClose }) => {
  ...
  const handleCarSubmit = async (e) => {
    ...
    if (selectedFiles.length > 0) {
      imageList = await Promise.all(selectedFiles.map((file) => fileToDataUrl(file)));
    }

    onAddCar({ ...carForm, images: imageList });
    ...
    <section id="admin" className="admin-section">
      <div className="admin-card">
        ...
        {!isLoggedIn ? (
          ...
          <>
            <p className="form-message success">Sesión iniciada. Añade coches al catálogo.</p>
            ...
            <form className="admin-form" onSubmit={handleCarSubmit}>
              <div className="form-grid">
                ...
                <div className="form-row">
                  ...
                  />
                  <small className="form-help">Seleccionadas: {selectedFiles.length}</small>
                </div>
              </div>
